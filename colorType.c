...
ColorType* traceRay(RayType *ray, SphereType* sphere, ColorType* backgroundColor){
    float A = pow(ray->dx,2) + pow(ray->dy,2) + pow(ray->dz,2);
    float B = 2* (ray->x - sphere->x) * ray->dx + 2* (ray->y - sphere->y) * ray->dy + 2* (ray->z - sphere->z) * ray->dz;
    float C = pow((ray->x - sphere->x), 2) + pow((ray->y - sphere->y), 2) + pow((ray->z - sphere->z), 2) - pow((sphere->r), 2);
